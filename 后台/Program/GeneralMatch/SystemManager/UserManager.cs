using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Drawing;
using System.Data;
using System.Linq;
using System.Text;
using System.Windows.Forms;
using ClientCommon;

namespace SystemManager
{
    public partial class UserManager : UserControl, IDataRefresh
    {
        public UserManager()
        {
            InitializeComponent();

            RefreshData();
        }

        public void AddRoleItems()
        {
            cbRoles.Items.Clear();
            DataTable dtRole = new RolePst(null).GetDataTable();
            foreach (DataRow dr in dtRole.Rows)
            {
                cbRoles.Items.Add(dr["Guid"], dr["Name"].ToString());
            }
        }

        #region IDataRefresh成员
        DataTable dt;
        public DataTable[] OperateTables
        {
            get
            {
                DataTable[] dts = new DataTable[1];
                dts[0] = dt;
                return dts;
            }
        }

        public void RefreshData()
        {
            AddRoleItems();
            dt = new ClientCommon.SystemUsersPst(null).GetDataTable();
            foreach (DataRow dr in dt.Rows)
            {
                ApplicationCommon.PasswordHelper ph = new ApplicationCommon.PasswordHelper();
                dr["PassWord"] = ph.DecryptString(dr["PassWord"].ToString());
            }
            dt.AcceptChanges();
            gcUsers.DataSource = dt;
        }

        public void SaveData()
        {
            ClientCommon.TableSave ts = new ClientCommon.TableSave(new ClientCommon.TableInfo(typeof(ClientCommon.SystemUsers),
                            typeof(ClientCommon.SystemUsersPst)), dt);
            ts.Save();

        }
        #endregion

        private void btSave_Click(object sender, EventArgs e)
        {
            SaveData();
        }

        private void btAdd_Click(object sender, EventArgs e)
        {
            DataRow dr = dt.NewRow();
            dr["Guid"] = Guid.NewGuid();
            dr["UserName"] = "New user";
            dr["PassWord"] = "123";
            dt.Rows.Add(dr);
        }

        private void btRefresh_Click(object sender, EventArgs e)
        {
            DataRefreshOperate dro = new DataRefreshOperate(this);
            dro.SaveDataOnRefresh();
        }

        private void btDelete_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            DataRow dr = gvUsers.GetFocusedDataRow();
            if (dr != null)
            {
                if (MessageBox.Show("是否删除该记录？", "询问", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    dr.Delete();
                }
            }
            else
            {
                MessageBox.Show("请选择一条需要删除的记录！");
            }
        }
    }
}
